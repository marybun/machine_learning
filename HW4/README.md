...
### i. Fitting Data
For a simple example of polynomial curve fitting with this dataset, check out [HW1](https://github.com/marybun/machine_learning/blob/main/HW1).
...
We build a 3 layer feed forward neural network with 1 neuron as the input and output, since our data are points. The hidden layer has 10 neurons. We use a ReLU activation function for the first 2 layers. We don't use it for the output layer, since we want to allow negative valued outputs.
...
Similarly, we fit the data using the same neural network, this time, with the middle 10 points as training and the rest as testing data. 
...
Similarly, we build a LSTM neural network and train and test over the same data.
